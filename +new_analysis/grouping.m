...
weighted_avg_stim_baseline = sum(weighted_stim_baseline, 1) ./ sum(total_num_trials, 1);
...
unit_group_indices = [for_units_animal_ids, all_unit_cluster_ids, for_units_days_from_learning];
...
for stim_idx=1:length(unique_stims)
    num_resp_units{stim_idx} = ap.groupfun(@sum, ...
        cat_all_resp_units{stim_idx},  unit_group_clusters_indices, []);
...
for stim_idx=1:length(unique_stims)
    group_unit_mean_post_stim{stim_idx} = ap.groupfun(@mean, ...
        cat_unit_mean_post_stim{stim_idx},  unit_group_clusters_indices, []);
end
...
for stim_idx=1:length(unique_stims)
    group_unit_norm_mean_post_stim{stim_idx} = ap.groupfun(@mean, ...
        cat_unit_norm_post_stim{stim_idx},  unit_group_clusters_indices, []);
end
...
for stim_idx=1:length(unique_stims)
    group_unit_std_post_stim{stim_idx} = ap.groupfun(@mean, ...
        cat_unit_std_post_stim{stim_idx},  unit_group_clusters_indices, []);
end
...
sgtitle('AM NORM MEAN post stim', 'FontSize', 20);
